# MS-4004 OpenCode 办公效率课程

> 基于 Microsoft MS-4004 官方课程内容，适配 OpenCode 本地化实现

**课程理念**：用自然语言与 AI 协作，提升办公效率

---

## 📖 课程介绍

本课程面向企业高管、销售人员、IT 专业人员等办公人群，通过 OpenCode Desktop 提升日常办公效率。

**课程特色**：
- ✅ **零基础友好**：不需要编程基础，用自然语言描述需求即可
- ✅ **本地运行**：使用 OpenCode Desktop，数据不出本地
- ✅ **实战导向**：10 个真实办公场景，50+ 个实验
- ✅ **递进式学习**：入门版提示词 → 优化版提示词，循序渐进
- ✅ **探索式学习**：AI 输出偏差是 feature，鼓励迭代调整
- ✅ **完整覆盖**：高管、销售、IT、市场、财务、HR、运营、沟通、客服、法务

---

## 📚 课程目录

| 模块 | 适用人群 | 实验内容 |
|------|---------|---------|
| **M01** | 企业高管、管理层 | 分析公司财务数据，生成高管简报和关键指标仪表板。 |
| **M02** | 销售团队、售前工程师 | 分析客户需求文档，生成定制化销售提案和 RFP 响应。 |
| **M03** | IT 项目经理、技术负责人 | 分析 IT 项目数据，生成项目报告、调查分析和 PPT 演示。 |
| **M04** | 市场团队、品牌专员 | 分析营销数据，生成 ROI 分析、AI 创意和文案优化。 |
| **M05** | 财务人员、分析师 | 分析财务数据，生成成本分析、合同对比和邮件起草。 |
| **M06** | 人力资源、招聘团队 | 分析 HR 数据，生成人员分析、政策问答和协作规划。 |
| **M07** | 运营团队、项目经理 | 分析运营数据，识别效率瓶颈和改进机会。 |
| **M08** | 全员 | 分析沟通数据，生成战略简报、效果分析和全员公告。 |
| **M09** | 客服团队 | 分析 90 天支持案例数据，识别 Top 问题、趋势和关键洞察。 |
| **M10** | 法务团队 | 研究 CCPA 和 CPRA 法规，生成法律指南和业务简报。 |

> 📄 **完整实验概览**：查看 [EXPERIMENTS-OVERVIEW.md](EXPERIMENTS-OVERVIEW.md) - 10 个场景、50+ 个实验的详细描述

---

## 🚀 快速开始

### 前置要求

- 安装 [OpenCode Desktop](https://github.com/openclaw/opencode)
- 配置 AI 模型（推荐：百炼 Code Plan / qwen3.5-plus）
- Python 3.8+（OpenCode 会自动管理依赖）

### 使用步骤

1. **克隆仓库**
   ```bash
   git clone https://github.com/cloudzun/ms-4004-opencode-course.git
   cd ms-4004-opencode-course/M01-executives
   ```

2. **打开 OpenCode Desktop**
   - 工作目录指向模块目录（如 `M01-executives/`）

3. **按照实验手册操作**
   - 打开 `README.md`
   - 复制提示词，粘贴到 OpenCode 对话框
   - 等待生成结果

---

## 💡 学习建议

### 1. 从简单提示词开始

每个实验都提供**入门版提示词**，建议先尝试这个版本：

```
请读取 Excel 文件，生成高管简报。
内容要求：执行摘要、关键洞察、同比分析、风险提示。
```

### 2. 根据结果迭代优化

如果效果不满意，使用**优化版提示词**或自定义调整：

```
请添加两张可视化图表：
1. 各类别 2024 vs 2025 收入对比柱状图
2. 各地区收入增长率对比条形图
```

### 3. 使用前置提示词设置规范

为了获得更一致的输出质量，建议在开始实验前先设置格式规范：

```
在开始后续任务之前，请先了解以下格式规范：

**字体要求**：
- 中文字体：微软雅黑
- 英文字体：Verdana

**语言要求**：
- 所有输出使用简体中文
- 专业术语保留英文原文

请在后续所有任务中记住并应用这些规范。
```

### 4. 鼓励迭代调整

**AI 的输出偏差是 feature，不是 bug**——你可以像和同事交流一样不断调整，直到满意为止。

**示例**：

| 轮次 | 你的提示词 | 结果 |
|------|-----------|------|
| 第一次 | "请生成一份销售提案" | 只有文字，没有数据表格 |
| 第二次 | "请添加 ROI 分析表格，展示不同规模酒店的投资回报" | 有表格，但数据不完整 |
| 第三次 | "请补充表格数据，包含小型/中型/大型酒店的成本和回报周期" | 完美 ✅ |

---

## 🎯 提示词技巧

好的提示词包含四个要素，能帮助 OpenCode 给出更准确的结果：

| 要素 | 说明 | 示例 |
|------|------|------|
| **目标** | 你想让 OpenCode 做什么 | "生成一份高管简报" |
| **背景** | 为什么需要这个，给谁看 | "为高级领导团队准备，用于季度汇报" |
| **来源** | 数据从哪里来 | "基于 Q3 销售数据 Excel 文件" |
| **要求** | 格式、结构、细节要求 | "包含执行摘要和关键洞察，保存为 Word 文档" |

---

## ❓ 常见问题

### 文件与格式问题

**Q1: 为什么生成了 `_v2` 版本的文件？**

**原因**：原文件已被打开或占用，OpenCode 无法覆盖。

**解决**：关闭已打开的文件，让 OpenCode 重新生成，或手动将 `_v2` 重命名。

**预防**：生成文件前先关闭目标文件。

---

**Q2: 图表中的中文显示为方框怎么办？**

**原因**：matplotlib 默认字体不支持中文。

**解决**：让 OpenCode 添加中文字体配置（Microsoft YaHei/SimHei），然后重新生成。

> 💡 **已优化**：本课程提示词已包含中文字体配置要求。

---

**Q3: 提示"Permission denied"或"Device or resource busy"怎么办？**

**原因**：Word/Excel 文件已被打开，无法写入。

**解决**：关闭已打开的文件，让 OpenCode 重新生成。

---

**Q4: PPT 生成的样式太简单怎么办？（M03）**

**原因**：VBA 宏生成的 PPT 只包含基本内容。

**解决**：使用 PowerPoint"设计师"工具自动美化，或手动应用模板。

**预期管理**：当前方案适合快速生成初稿，正式场合建议手动美化。

---

### 内容与语言问题

**Q5: 生成的内容和格式不符合要求怎么办？**

**这是正常的！** OpenCode 根据提示词生成内容，如果结果不满意：

1. **明确指出问题**：如"ROI 分析表格缺失"
2. **告诉它如何修改**：如"请添加表格展示不同规模酒店的投资回报"
3. **迭代优化**：可能需要 2-3 轮对话

> 💡 **鼓励**：AI 的输出偏差是 feature，不是 bug。像和同事交流一样不断调整，直到满意。

---

**Q6: OpenCode 生成了英文内容怎么办？**

**原因**：提示词中没有明确指定语言。

**解决**：让 OpenCode 使用简体中文重新生成，专业术语保留英文原文。

**预防**：在提示词开头明确"使用简体中文"。

---

### 模块特定问题

**Q7: 合同审查的结果准确吗？（M05、M10）**

**说明**：AI 生成的合同摘要基于训练数据和文本分析，适用于初步审查。

**最佳实践**：
- ✅ 学习阶段：使用 AI 生成框架
- ⚖️ 实际应用：AI 生成 + 资深法务审核 + 官方来源验证

---

**Q8: 法规研究的信息如何验证？（M10）**

**说明**：AI 生成的法规信息可能不准确或不完整。

**最佳实践**：
1. 使用网络搜索功能获取最新法规
2. 交叉检查官方来源（.gov 网站）
3. 标注不确定性

---

**Q9: 风险评估矩阵的优先级如何设定？（M07、M10）**

**说明**：风险优先级（高/中/低）基于可能性和影响的组合。

**建议**：
- 🔴 高优先级：高可能性 + 高影响（30 天内处理）
- 🟡 中优先级：中等可能性或影响（60 天内处理）
- 🟢 低优先级：低可能性 + 低影响（90 天内处理）

---

**Q10: 客服数据分析的 Top 问题如何识别？（M09）**

**说明**：Top 问题基于案例数量、严重程度和升级率综合识别。

**建议**：结合业务场景调整权重，如高严重性问题优先处理。

---

**Q11: HR 数据分析的隐私如何保护？（M06）**

**说明**：HR 数据包含敏感个人信息。

**最佳实践**：
- 数据脱敏处理（移除姓名、工号等）
- 仅分析聚合数据
- 遵守公司数据保护政策

---

**Q12: 营销创意生成的内容如何使用？（M04）**

**说明**：AI 生成的创意适合头脑风暴和初稿。

**建议**：
- 作为创意起点，人工优化
- 结合品牌调性调整
- A/B 测试验证效果

---

## 🔗 相关资源

### 官方文档
- [OpenCode 官方文档](https://github.com/openclaw/opencode)
- [Microsoft MS-4004 原版课程](https://github.com/MicrosoftLearning/MS-4004-Empower-workforce-copilot-use-cases)
- [OpenClaw 社区](https://clawhub.ai)

### 技术参考
- [python-docx 库文档](https://python-docx.readthedocs.io/)（Word 文档处理）
- [openpyxl 库文档](https://openpyxl.readthedocs.io/)（Excel 文件操作）
- [matplotlib 库文档](https://matplotlib.org/)（图表生成）
- [python-pptx 库文档](https://python-pptx.readthedocs.io/)（PPT 生成技术参考）

### 技能扩展
- [office-automation-skill](https://github.com/texiaoyao/office-automation-skill)（Office 文档处理 skill）
- [markitdown-skill](https://github.com/karmanverma/markitdown-skill)（文档转 Markdown skill）

---

## 📄 许可证

MIT License

---

## 🙏 致谢

- 基于 Microsoft MS-4004 官方课程内容改编
- 感谢 OpenClaw 社区提供的工具和平台支持

---

**Made with ❤️ for OpenCode Community**
